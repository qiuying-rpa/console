    ...
    for condition in conditions:
        if condition["middle"] == "truthy":
            ...
            condition_list.append(
                " ".join([condition["left"], condition["middle"], condition["right"]])
            )
...
def analysis_nodes(nodes, layer=1):
    ...
    for node in nodes:
        ...
        if node["func"] == "exec-conditional":
            ...
            condition_code = generate_condition_code(
                condition_type, node["kwargs"]["conditions"]
            )
            code_list.append(f"{prefix_str}if {condition_code}:")
            true_code_list = (
                analysis_nodes(node["kwargs"]["executionTruthy"], layer=layer + 1)
            ...
            false_code_list = (
                analysis_nodes(node["kwargs"]["executionFalsy"], layer=layer + 1)
            ...
            if circular_type == "for":
                ...
                condition_type = (
                    "and" if node["kwargs"]["type"].startswith("all") else "or"
                )
                condition_code = generate_condition_code(
                    condition_type, node["kwargs"]["conditions"]
                )
            ...
            code_list += exec_code
        elif node["func"] == "base.set_variable":
            python_row_code = (
                prefix_str
                + node["kwargs"]["variable"]
                + " = "
                + node["kwargs"]["variableValue"]
            )
            code_list.append(python_row_code)
        elif node["func"] == "base.print_log":
            python_row_code = prefix_str + "print(" + node["kwargs"]["content"] + ")"
            code_list.append(python_row_code)
        else:
