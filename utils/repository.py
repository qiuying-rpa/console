...
def find_one_by(model, prop_name, prop_value):
    ...
    record = __db.session.execute(__db.select(model).filter_by(**{prop_name: prop_value})).first()
    return record[0] if record else None


def find_many(model, many_ids):
    """Find many"""
    return list(map(lambda x: x[0], __db.session.execute(__db.select(model).filter(model.id.in_(many_ids))).all()))


def find_many_by(model, prop_name, prop_value):
    """Find many by a certain prop"""
    return list(map(lambda x: x[0], __db.session.execute(__db.select(model).filter_by(**{prop_name: prop_value})).all()))


def find_other_with_same(model, the_id, prop_name, prop_value):
    """Find the other one with the same value of a certain prop"""
    record = __db.session.execute(__db.select(model).filter_by(
